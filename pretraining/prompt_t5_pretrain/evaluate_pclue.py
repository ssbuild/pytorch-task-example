...
def rouge_l_zh(target, pred):
    ...
    if not(isinstance(target, str) or isinstance(pred, str)):
        logging.info("target或pred为非字符串！请检查!")
        return
    else:
        rouge = Rouge()
...
def evaluate_pclue_file_fn(predict_file,target_file):
    predict_lines = open(predict_file, 'r').readlines()
    target_lines = open(target_file, 'r').readlines()

    return evaluate_pclue_fn(predict_file,target_file)

def evaluate_pclue_fn(predict_lines,target_lines):
    """
    计算pclue的成绩
